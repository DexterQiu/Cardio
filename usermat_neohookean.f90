...
!   PROPS(2) = volumetric penalty parameter, d
...
!     Material properties
      ...
      IF (NPROPS .GE. 3) THEN
        ...
        END IF
      END IF
      DPEN_EFF = DPEN / MAX(RAMP, 1.0D-12)

!     2nd PK stress (Total Lagrangian, penalty)
!     S = mu * I + (2/d_eff) * (J - 1) * J * Cinv
      CALL NEOHOOK_S2PK(CMAT, MU, DPEN_EFF, S2PK, TRC)
...
!     Consistent tangent via numerical differentiation in Green-Lagrange strain
      ...
      DO JIDX = 1,6
        ...
        END IF
        CALL NEOHOOK_S2PK(CWORK, MU, DPEN_EFF, SPERT, TRC)
        ...
        ELSE
          ...
          CWORK(3,1) = CWORK(3,1) - DC
        END IF
        CALL NEOHOOK_S2PK(CWORK, MU, DPEN_EFF, SMINUS, TRC)
...
! ---------------------------------------------------------------------------
      SUBROUTINE NEOHOOK_S2PK(CMAT, MU, DPEN, S2PK, TRC)
      IMPLICIT NONE
      DOUBLE PRECISION CMAT(3,3), S2PK(3,3)
      DOUBLE PRECISION MU, DPEN, TRC
      ...
      S2PK = MU * I3 + (2.0D0/DPEN) * (J - 1.0D0) * J * CMATINV
